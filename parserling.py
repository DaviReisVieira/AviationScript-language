...
class Parser():
    ...
    def parse(self):
        ...
        @self.pg.production("takeoff : TAKEOFF OPEN_BRACE AIRCRAFT aircraftName RUNWAY IDENTIFIER FLAPS NUMBER SPEED NUMBER ALTITUDE NUMBER CLOSE_BRACE")
        def takeoff(p):
            return Takeoff(p[3], p[5], p[7], p[9], p[11])

        # aircraftName
        @self.pg.production("aircraftName : STRING")
        @self.pg.production("aircraftName : IDENTIFIER")
        @self.pg.production("aircraftName : IDENTIFIER OPEN_BRACKET STRING CLOSE_BRACKET")
        def aircraftName(p):
            if p[0].gettokentype() == "STRING":
                ...
                if len(p) == 1:
                    ...
                    string = String(p[2].getstr())
                    print(symbolTable.table)
                    return Array(identifier, string).eval()
        ...
        def factor(p):
            if len(p) == 1:
                ...
                if p[0].gettokentype() == "IDENTIFIER" and p[1].gettokentype() == "OPEN_BRACKET":
                    identifier = Identifier(p[0].getstr())
                    string = String(p[2].getstr())
